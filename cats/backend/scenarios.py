...
class GetScenarios():
    ...
    def __repr__(self):
        return str([x for x in self.scenarios.keys()])
    ...
    def read_config(self, filepath):
        ...
        try:
            self.results_folder = os.path.join(dirname, os.pathconfig["results"]["folder"])
        except:
        ...
        try:
            ...
            raise KeyError("No years were provided for model runs.  This needs to be set for each scenario, or 'all' needs to be defined.")
    ...
    def _year_parse(self, string):
        try:
            ...
            print("Turning model years to run into a list...")
        ...
        if "," in string:
            ...
            try:
                ...
                raise Exception("Format for years in the config file was invalid. {}".format(string))
            years = [x for x in range(minimum, maximum)]
        ...
        for year in years:
            try:
                ...
                for x in additional_range:
                    return_years.append(x)
    ...
    def validate_scenarios(self, scenarios):
        if scenarios == "all":
            try:
                ...
                raise Exception("The scenario folder '{}' cannot be found".format(self.scenario_folder))
        else:
